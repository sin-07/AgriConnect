...
import {
  ...
  useGsapParallax,
  useGsapTextReveal,
  useGsapSectionHeader,
} from "@/hooks/useAnimations";

// ─── Page ──────────────────────────────────────────────────────────────────────
export default function HomePage() {
  const heroRef = useGsapHero();
  const featuresRef = useGsapStaggerCards(".gsap-feature-card");
  const howItWorksRef = useGsapScrollReveal(".gsap-step", { y: 60, stagger: 0.2, duration: 0.8 });
  const categoriesRef = useGsapScrollReveal(".gsap-cat-pill", { y: 40, stagger: 0.05, duration: 0.5 });
  const catTitleRef = useGsapSectionHeader();
  const catHeadingRef = useGsapTextReveal();
  const ctaRef = useGsapScrollReveal(".gsap-cta-card", { y: 80, stagger: 0.2, duration: 0.9 });
  const featuresTitleRef = useGsapTextReveal();
  const howTitleRef = useGsapTextReveal();
  const parallaxBlob = useGsapParallax(0.4);

  return (
    <div className="overflow-x-hidden">

      {/* ═══ HERO ════════════════════════════════════════════════════════════ */}
      <section ref={heroRef} className="relative min-h-[92vh] flex items-center bg-gradient-to-br from-primary-950 via-primary-800 to-emerald-700 overflow-hidden">
        {/* Decorative blobs */}
        <div className="pointer-events-none absolute inset-0">
          <div className="hero-blob absolute -top-40 -right-40 w-[600px] h-[600px] rounded-full bg-emerald-500/10 blur-3xl" />
          <div className="hero-blob absolute bottom-0 -left-40 w-[500px] h-[500px] rounded-full bg-primary-400/10 blur-3xl" />
          <div ref={parallaxBlob} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] rounded-full bg-white/3 blur-3xl" />
          {/* Floating wheat icons */}
          <GiWheat className="hero-wheat-float absolute top-[12%] right-[10%] text-7xl text-white/8" />
          <GiWheat className="hero-wheat-float absolute bottom-[18%] left-[6%] text-5xl text-white/8" />
          <GiWheat className="hero-wheat-float absolute top-[55%] right-[20%] text-4xl text-white/6" />
        </div>

        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 grid lg:grid-cols-2 gap-16 items-center">
          {/* Left — text */}
          <div>
            <div className="hero-badge inline-flex items-center gap-2 bg-white/10 backdrop-blur border border-white/20 text-emerald-200 text-xs font-semibold px-4 py-1.5 rounded-full mb-8 tracking-wider uppercase">
            ...
            <h1 className="font-display text-5xl sm:text-6xl lg:text-7xl font-extrabold text-white leading-[1.1] tracking-tight mb-6">
              <span className="hero-title-line inline-block">Farm Fresh,</span>
              <br />
              <span className="hero-gradient-text text-transparent bg-clip-text bg-gradient-to-r from-emerald-300 to-lime-300">
                Zero Brokers.
              </span>
            </h1>
            <p className="hero-desc text-lg text-primary-100 max-w-lg mb-10 leading-relaxed">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Link
                href="/marketplace"
                className="hero-cta inline-flex items-center justify-center gap-2 bg-white text-primary-700 px-8 py-4 rounded-2xl font-bold text-base hover:bg-emerald-50 hover:shadow-xl hover:shadow-white/20 hover:-translate-y-0.5 transition-all duration-200"
              >
                <FiShoppingBag /> Browse Marketplace
              </Link>
              <Link
                href="/register"
                className="hero-cta inline-flex items-center justify-center gap-2 border-2 border-white/40 text-white px-8 py-4 rounded-2xl font-bold text-base hover:bg-white/10 hover:border-white/70 hover:-translate-y-0.5 transition-all duration-200"
              >
            ...
            <div className="flex items-center gap-6 mt-12">
              {[
                ...
                <div key={t.text} className="hero-trust-item flex items-center gap-1.5 text-emerald-200 text-sm font-medium">
                  {t.icon} {t.text}
                </div>
              ))}
            </div>
          ...
          <div className="hidden lg:flex justify-center">
            <div className="relative w-80">
              {/* Back card */}
              <div className="hero-card-back absolute -top-6 -right-6 w-full h-full bg-white/10 backdrop-blur rounded-3xl border border-white/20" />
              {/* Front card */}
              <div className="hero-card-front relative bg-white/15 backdrop-blur-xl border border-white/25 rounded-3xl p-8 shadow-2xl">
                ...
                {[
                  ...
                  <div key={p.name} className="hero-product-row flex items-center justify-between py-3 border-b border-white/10 last:border-0">
                    <div>
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <h2 ref={featuresTitleRef as React.RefObject<HTMLHeadingElement>} className="font-display text-4xl font-extrabold text-gray-900 mb-4">Built for those who grow food</h2>
          ...
          >
            {[
              ...
              <div
                key={f.title}
                className="gsap-card gsap-feature-card group relative bg-white border border-gray-100 rounded-3xl p-8 shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-400 overflow-hidden"
              >
      ...
      <section className="py-24 bg-[#f7f9f4]">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <h2 ref={howTitleRef as React.RefObject<HTMLHeadingElement>} className="font-display text-4xl font-extrabold text-gray-900">How it works</h2>
          </div>
          ...
          <div ref={howItWorksRef} className="relative">
            ...
            <div className="grid md:grid-cols-3 gap-10">
              {[
                ...
                <div
                  key={s.step}
                  className="gsap-step text-center"
                >
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div ref={catTitleRef} className="text-center mb-14">
            <span className="page-header-badge text-xs font-bold uppercase tracking-widest text-primary-500 mb-3 block">Browse by category</span>
            <h2 ref={catHeadingRef as React.RefObject<HTMLHeadingElement>} className="page-header-title font-display text-4xl font-extrabold text-gray-900">What&apos;s on the marketplace</h2>
          </div>

          <div ref={categoriesRef} className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            {[
              ...
              <Link
                ...
                className={`gsap-cat-pill border rounded-2xl px-4 py-5 text-center text-sm font-semibold transition-all duration-200 hover:-translate-y-1 hover:shadow-md
                  ${c.label === "View All"
      ...
      >
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-8">
            {/* Farmer card */}
            <div className="gsap-cta-card bg-white/10 backdrop-blur border border-white/20 rounded-3xl p-10 hover:bg-white/15 hover:scale-[1.02] transition-all duration-300">
